...
function challengeStatus(state = {}) {
  if (state.phase === 'open') {
    ...
    return { badge:'DISPONÍVEL', title:`Parte ${state.part} disponível agora`, detail:`${state.round?.questionCount || 5} perguntas · ${challengeTimeLabel(challengeTimeSeconds(state))} no total`, tone:'#5DE1FF', color:'#071B3A' };
  }
...
function ChallengeGame({ challengeState, onFinish, onExit, onStart, onSaveAnswer }) {
  ...
  const sendCurrent = React.useCallback(async (allowBlank = false) => {
    ...
    try {
      ...
      setQuestionIndex(index => index + 1); setAnswer(''); setPhase('transition');
